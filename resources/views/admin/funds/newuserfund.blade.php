...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Fix new user registration fund</h1>
</div>
        ...
        <div class="col-8 col-xl-10">
            <div class="card shadow">
                ...
                <div class="alert alert-success fade show m-3" role="alert">
                    Current new user registration fund is: <span>{{ $regfund->amount }}</span>
                </div>
                @else
                <div class="alert alert-warning fade show m-3" role="alert">
                    <strong>New user registration fund is not fixed</strong>
                </div>
                @endif
                ...
                <div class="card-body">
                    @if(!$regfund)
                    <form action="{{ route('funds.regfix') }}" method="post">
                        @else
                        <form action="{{ route('funds.regchange') }}" method="post">
                            @endif
                            @csrf
                            <div>
                                <div class="mb-3">
                                    ...
                                    @enderror
                                    @else
                                    <label for="fix_tax" class="form-label">Change new user registration amount?<span class="text-danger">*</span></label>
